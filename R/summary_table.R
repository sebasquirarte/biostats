...
#'
#' @return A data frame with one row per variable containing summary statistics and test results.
#'
...
#'                                  exclude = c('subject_id', 'visit'))
#'
#'# Grouped summary for each tratment group with all stats and effect size
#'clinical_summary <- summary_table(clinical_df,
#'                                  group_var = 'treatment',
#'                                  all_stats = TRUE,
#'                                  effect_size = TRUE,
...
summary_table <- function(data,
  ...
  if (use_groups) {
    if (!group_var %in% names(data)) stop("The grouping variable '", group_var, "' is not found in the data.")
    if (length(unique(data[[group_var]])) != 2) stop("When using group_var, it must have exactly two groups.")
  }
