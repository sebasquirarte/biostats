                 ...
                 if(grepl("One-way ANOVA", name)) {
                    ...
                    for (i in 1:nrow(comparisons)) {
                      ...
                      cat(sprintf("%-20s %8.3f %8.3f %8.3f %8s%s\n",
                        ...
                        fit.emmeans <- emmeans(model, as.formula(paste("~", independent_var)))
              ...
              tryCatch({
                if (condition == TRUE) {
                    ...
                    # Pairwise t-tests with specified adjustment
                      post_hoc <- suppressWarnings(pairwise.t.test(data[[dependent_var]], data[[independent_var]], p.adjust.method = method))
                      cat(sprintf("Pairwise t-tests (Correction: %s):\n", tools::toTitleCase(method)))
                    } else {
                        # Paired pairwise t-tests with specified adjustment
                        post_hoc <- suppressWarnings(pairwise.t.test(data[[dependent_var]], data[[independent_var]], paired = TRUE,p.adjust.method = method))
